...
// Michael: Simplified debug logging
const debugEmbeddings = process.env.DEBUG_MEMORY_EMBEDDINGS === "true";
...
export async function resolveGeminiEmbeddingClient(
  ...
  // Michael: Simplified API key handling
  const apiKey = remoteApiKey || process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error("No API key found for provider: gemini. Set GOOGLE_API_KEY or GEMINI_API_KEY environment variable.");
  }
