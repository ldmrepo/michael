// @ts-nocheck - Temporarily disable type checking during Moltbot migration
...
import type { MichaelMemoryConfig, MemorySearchConfig } from "./config.js";
import { resolveUserPath } from "./config.js";
// Michael: Using console for logging (TODO: migrate to Michael's log utility)
import {
...
// Michael: Only "memory" source (no sessions)
...
const INDEX_CACHE_MAX_SIZE = 10;
const INDEX_CACHE = new Map<string, MemoryIndexManager>();

/**
 * Remove oldest entries from INDEX_CACHE if over max size
 */
function pruneIndexCache(): void {
  if (INDEX_CACHE.size <= INDEX_CACHE_MAX_SIZE) return;

  // Map maintains insertion order, so first entries are oldest
  const keysToRemove = Array.from(INDEX_CACHE.keys()).slice(
    0,
    INDEX_CACHE.size - INDEX_CACHE_MAX_SIZE
  );
  for (const key of keysToRemove) {
    const manager = INDEX_CACHE.get(key);
    if (manager) {
      // Close the manager before removing from cache
      manager.close().catch(() => {});
    }
...
export class MemoryIndexManager {
  ...
  private readonly cfg: MichaelMemoryConfig;
  private readonly dataDir: string; // Michael: single data directory
  private readonly workspaceDir: string;
  private readonly agentId: string; // Configurable agent ID (default: "michael")
  private readonly settings: MemorySearchConfig;
  ...
  // Michael: Simplified factory method (single user, no agent scope)
  static async get(params: {
    cfg: MichaelMemoryConfig;
    dataDir: string;
    agentId?: string; // Optional agent ID (default: "michael")
  }): Promise<MemoryIndexManager | null> {
    const { cfg, dataDir, agentId = "michael" } = params;
    const settings = cfg.memory.search;
    if (!settings) return null;

    const workspaceDir = dataDir; // Michael: workspace = data directory
    const embeddingConfig = {
      provider: settings.provider,
      model: settings.model,
      fallback: settings.fallback,
    };
    const key = `${agentId}:${workspaceDir}:${JSON.stringify(embeddingConfig)}`;
    ...
    pruneIndexCache(); // Prevent memory leak from unbounded cache growth
    return manager;
  }
  ...
  async warmSession(sessionKey?: string): Promise<void> {
    // Michael: Session warmup not needed (no session concept)
    return;
  }
  ...
  async search(
    ...
    // Michael: Only check dirty (no sessionsDirty)
  ...
  private dropVectorTable(): void {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      console.debug(`Failed to drop ${VECTOR_TABLE}: ${message}`);
    }
  }
  ...
  private scheduleSessionDirty(sessionFile: string) {
    ...
    this.sessionWatchTimer = setTimeout(() => {
      ...
      void this.processSessionDeltaBatch().catch((err) => {
        console.warn(`memory session delta failed: ${String(err)}`);
      });
  ...
  private async processSessionDeltaBatch(): Promise<void> {
    ...
    if (shouldSync) {
      void this.sync({ reason: "session-delta" }).catch((err) => {
        console.warn(`memory sync failed (session-delta): ${String(err)}`);
      });
    }
  }
  ...
  private isSessionFileForAgent(sessionFile: string): boolean {
    // Michael: No session files, always return false
    return false;
  }
  ...
  private async syncMemoryFiles(params: {
    ...
    console.debug("memory sync: indexing memory files", {
  ...
  private async syncSessionFiles(_params: {
    needsFullReindex: boolean;
    progress?: MemorySyncProgressState;
  }) {
    // Michael: Session file syncing not needed (no session concept)
  }
  ...
  private async runSync(params?: {
    ...
    try {
      ...
      // Michael: Session file syncing disabled (no session concept)
    } catch (err) {
  ...
  private async activateFallbackProvider(reason: string): Promise<boolean> {
    ...
    const fallbackResult = await createEmbeddingProvider({
      config: this.cfg,
      agentDir: this.dataDir, // Michael: single data directory
      provider: fallback,
  ...
  private async buildSessionEntry(absPath: string): Promise<SessionFileEntry | null> {
    try {
      ...
      console.debug(`Failed reading session file ${absPath}: ${String(err)}`);
      return null;
    }
  }
    ...
    const batchResult = await this.runBatchWithFallback({
      ...
      run: async () =>
        await runOpenAiEmbeddingBatches({
          ...
          pollIntervalMs: this.batch.pollIntervalMs,
          timeoutMs: this.batch.timeoutMs,
          debug: (message, data) => console.debug(message, { ...data, source, chunks: chunks.length }),
        }),
  ...
  private async embedBatchWithRetry(texts: string[]): Promise<number[][]> {
    ...
    while (true) {
      try {
        ...
        console.debug("memory embeddings: batch start", {
          provider: this.provider.id,
          items: texts.length,
          timeoutMs,
        });
        ...
        console.warn(`memory embeddings rate limited; retrying in ${waitMs}ms`);
  ...
  private async embedQueryWithTimeout(text: string): Promise<number[]> {
    ...
    console.debug("memory embeddings: query start", { provider: this.provider.id, timeoutMs });
    return await this.withTimeout(
  ...
  private async resetBatchFailureCount(): Promise<void> {
    await this.withBatchFailureLock(async () => {
      if (this.batchFailureCount > 0) {
        console.debug("memory embeddings: batch recovered; resetting failure count");
      }
  ...
  private async runBatchWithTimeoutRetry<T>(params: {
    ...
    try {
      ...
      if (this.isBatchTimeoutError(message)) {
        console.warn(`memory embeddings: ${params.provider} batch timed out; retrying once`);
        try {
          return await params.run();
        } catch (retryErr) {
